...
def main():
    st.set_page_config(
        page_title="Diabetes Prediction", page_icon="⚕️", layout="centered"
    )
    ...
    if submitted:
        ...
        result_placeholder.markdown(
            f"""<h4 align="center" style="color: {"lightred" if result else "skyblue"}; margin-block: 15px; border: solid {"#e50914" if result else "#3f79d7"} 1px; border-radius: 10px; padding: 20px; margin-inline: auto; text-align: center;">{"Diabetes suspected." if result else "Diabetes not suspected."}</h4>""",
            unsafe_allow_html=True,
        )
    else:
        result_placeholder.markdown(
